...
Do not include any other text.`
        ...
        if (responseText.startsWith('APPROVE')) {
            ...
            const reason = reasonMatch ? reasonMatch[1].trim() : 'Message does not meet community guidelines'
            ...
            if (__DEV__) {
                console.warn('Sentinel: Unclear response, defaulting to approval:', responseText)
            }
            return { isApproved: true }
        }
    } catch (error) {
