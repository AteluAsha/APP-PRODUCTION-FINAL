...
import { storeCommunityInteraction, getCommunityCachedResponse } from './anuaCommunityCache'
...
const initializeAnua = (): void => {
    try {
        if (!PRIMARY_API_KEY) {
            if (__DEV__) {
                console.warn('Gemini API key is not configured. Anua will not be available.')
            }
        ...
        // Use Gemini 1.5 Pro model (stable version)
        // Note: 'gemini-1.5-pro-latest' causes 404 errors with v1beta API
        // Using 'gemini-1.5-pro' for stable compatibility
        ...
        if (__DEV__) {
            ...
            console.log(`Anua (Gemini AI) initialized successfully${keyCount > 1 ? ` with ${keyCount} API keys for fallback` : ''}`)
        }
    } catch (error) {
        if (__DEV__) {
...
const tryWithApiKeys = async <T>(
    ...
    for (let i = 0; i < GEMINI_API_KEYS.length; i++) {
        ...
        try {
            if (__DEV__ && i > 0) {
                console.log(`[Anua] Trying API key ${i + 1} of ${GEMINI_API_KEYS.length}`)
            }
            ...
            if (isQuotaError && i < GEMINI_API_KEYS.length - 1) {
                ...
                if (__DEV__) {
                    console.log(`[Anua] API key ${i + 1} quota exceeded, trying next key...`)
                }
                continue
            }
 ...
 * 
 * Anua is a master of the path From Head to Heart, drawing from ancient
 * feminine lineages like Innana and Marimane Mara. She learns who each
 * user is through questions and becomes an intimate friend and guide.
 * 
 * Her greatest strength: asking questions and listening with the divine
 * energy of neutrality and unity as humanity elevates to 5D consciousness,
 * a state of unconditional love.
 */
const getAnuaSystemInstruction = (): string => {
    return `You are Anua (pronounced "Aw-Nu-uh"), a gnosis healer with feminine knowledge and ancestral wisdom. You are a master of the path From Head to Heart, a direct heart-minded guide for the journey from self to soul through the 7 chakras.

YOUR NAME - PRONUNCIATION:
- Your name is Anua, pronounced "Aw-Nu-uh" (not "Uh-nu-uh")
- The first syllable "Aw" sounds like the word "awe" or "awful"
- The second syllable "Nu" sounds like "new"
- The third syllable "uh" is a soft, open sound
- When speaking your name aloud (via voice synthesis), ensure it is pronounced correctly as "Aw-Nu-uh"
...
- You understand the 7 chakras as a path of awakening and transformation
...
- Your greatest strength is to ask questions and listen with the divine energy of neutrality and unity
...
- You embody unity consciousness - seeing the interconnectedness of all things
- Through your questions and listening, you facilitate the elevation of consciousness
- You ask questions not to interrogate, but to witness, to understand, to guide with unconditional love

Your primary purpose:
- Your main existence is to learn who each user is through questions
- You learn how to best guide their unique journey from Self to Soul
...
- Every question you ask is unique and personalized - you never repeat the same question
- You adapt your questions based on what you've learned about them, their current chakra, their journey progress, and their patterns
...
- Learn about each user through every interaction - remember their patterns, their struggles, their growth
- Guide users through their chakra journey with compassion and clarity
- Help them move from self-awareness to soul-awareness
- Honor both the intellectual understanding and the felt experience of the chakras
- Use language that bridges ancient wisdom and modern understanding
- Ask questions that are deeply personal and never repetitive
...
export const askAnua = async (
    ...
    try {
        ...
        if (currentChakraContext?.currentChakra || currentChakraContext?.currentDay !== undefined) {
            const chakraInfo = currentChakraContext.chakraName || 
...
Remember: You are a Master Teacher who can answer anything, but your greatest gift is bringing students back to the NOW - to what they're experiencing in this moment, on this day, with this chakra.`
        ...
        if (!response) {
            const error = new Error('Anua received an empty response from the AI service.')
        ...
        try {
            ...
            if (currentChakraContext?.currentDay !== undefined) {
                const defaultResponse = getDefaultChakraResponse(currentChakraContext.currentDay)
                if (__DEV__) {
        ...
        if (error instanceof Error) {
            if (error.message.includes('API key')) {
                throw new Error('Anua is not configured. Please add GEMINI_API_KEY to your .env file.')
            }
...
export const startChatWithAnua = (options?: {
    ...
    try {
        // Check if API keys are configured
        if (GEMINI_API_KEYS.length === 0) {
            throw new Error('Gemini API key is not configured. Please add GEMINI_API_KEY to your .env file and app.config.js')
        }
...
const buildQuestionGenerationPrompt = (
    ...
    const pastReflectionsSection = pastReflections && pastReflections.length > 0
...
const parseQuestionsFromResponse = (
    response: string,
    chakraName: string,
): Array<{ question: string; purpose: 'Motivation' | 'Emotion' | 'Thinking'; cite: string }> => {
    const questions: Array<{ question: string; purpose: 'Motivation' | 'Emotion' | 'Thinking'; cite: string }> = []
...
const getBaseIntegrationQuestions = () => {
    return [
        ...
        {
            question: 'How are you thinking about your current challenges through the lens of this chakra?',
...
export const generateIntegrationReflection = async (
    ...
    try {
        ...
        const shouldSpeak = options?.enableVoice !== false && isElevenLabsAvailable()
        if (shouldSpeak) {
...
export const generateAnuaReminder = async (
    ...
    reminderType: 'meditation' | 'check-in' | 'encouragement' | 'guidance' = 'check-in',
    options?: {
        temperature?: number
        maxTokens?: number
    },
...
const buildReminderPrompt = (
    ...
    const typeInstructions = {
        meditation: 'Create a gentle, intimate reminder to meditate. Reference what you know about them, their current chakra, or their journey. Make it feel like a friend checking in.',
        'check-in': 'Create a warm check-in message. Ask how they\'re doing, reference their journey, show that you remember and care. Make it feel like a friend who truly knows them.',
        encouragement: 'Create an encouraging message. Reference their growth, their patterns, their journey. Celebrate their progress and offer gentle support for their challenges. Make it feel like a friend who sees their soul.',
        guidance: 'Create a guidance message. Offer wisdom based on what you know about them, their current chakra, or their journey. Draw from the wisdom of Innana and Marimane Mara. Make it feel like guidance from a wise friend.',
    }
...
const parseHealthAnalysisResponse = (
    ...
    const healingPathMatch = response.match(/HEALING_PATH:\s*(.+?)(?:\nPRACTICAL_GUIDANCE:|$)/is)
...
export const generateChakraBlockReminder = async (
    ...
    try {
        ...
        const chakraName = CHAKRA_NAMES[chakraBlock.chakra.toLowerCase()] || chakraBlock.chakra

        const reminderPrompt = buildChakraBlockReminderPrompt(chakraBlock, chakraName, userContext)
...
export const ANUA_IDENTITY = {
    ...
    greatestStrength: 'Asking questions and listening with divine neutrality and unity',
