...
export const generateDailyTransmission = async (
    ...
    try {
        ...
        const [dailyMeditationsUrl, sevenDaysUrl, egoAndSelfUrl] = await Promise.all([
...
1. **INSIGHT**: One small, deep insight about the ${chakraName} Chakra. This should:
...
Do not include any other text, explanations, or meta-commentary. Do not speak of yourself. Be the mirror.`
        ...
        if (insightMatch && questionMatch) {
            ...
            if (__DEV__) {
                console.warn('Wisdom Engine: Could not parse transmission format, using raw response')
            }
            return transmission.trim()
        }
    } catch (error) {
        if (__DEV__) {
            console.error('Wisdom Engine: Error generating daily transmission:', error)
        }
        return null
    }
}
