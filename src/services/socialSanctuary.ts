...
import { collection, addDoc, query, where, orderBy, limit, getDocs, Timestamp, onSnapshot, type Unsubscribe } from 'firebase/firestore'
...
export const getReflectionsForDay = async (
    ...
    try {
        ...
        const isExpectedError = errorMessage.includes('permission') || 
        ...
        if (error instanceof Error) {
            if (isExpectedError) {
                if (__DEV__) {
                    console.warn('Social Sanctuary collection may not exist yet or has permission issues. Returning empty array.')
                }
                return []
            }
        }
...
export const getTopReflections = async (
    ...
    try {
        ...
        if (__DEV__) {
            console.error('Error fetching top reflections from Social Sanctuary:', error)
        }
        // If it's a permissions error, collection doesn't exist, or index is missing, return empty array
        if (error instanceof Error) {
            if (isExpectedError) {
                if (__DEV__) {
                    console.warn('Social Sanctuary collection may not exist yet, has permission issues, or requires an index. Returning empty array.')
                }
                return []
            }
        }
...
export const subscribeToReflections = (
    ...
    try {
        ...
        if (!db) {
            if (__DEV__) {
                console.warn('Firebase is not initialized. Cannot subscribe to reflections.')
            }
            return null
        }
