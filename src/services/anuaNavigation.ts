...
export const parseNavigationRequest = async (
    ...
    try {
        ...
        const directMatch = NAVIGATION_ROUTES[lowerRequest as keyof typeof NAVIGATION_ROUTES]
        if (directMatch) {
            return directMatch
        }
...
export const useAnuaNavigation = (routerInstance: typeof router) => {
    return {
        navigate: (userRequest: string) => navigateWithAnua(userRequest, routerInstance),
