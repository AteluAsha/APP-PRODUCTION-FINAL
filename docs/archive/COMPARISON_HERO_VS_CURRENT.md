...
| Web dev                | (none)                                                                | if (Platform.OS === "web" && **DEV**) return CaptureAll                                                                                                                                                                                                                                                                              | **CHANGED.**                                                                             |
...
- **Dev reset:** In **DEV**, every load can clear journey and first-launch storage (when buildId changes or DEV_FORCE_TRIAL_HERO). That forces “first launch” and can change which screen appears first; good for testing, can surprise if you expect persisted state.
...
3. **ChakraHome + timegate + first launch**
   ...
   - **Suggestion:** Trace one path: cold start → dev reset (if **DEV**) → first paint of ChakraHome → value of `isFirstLaunch` and `showWelcomeModal` and `showWaitingScreen`. Confirm they match the intended hero flow.
