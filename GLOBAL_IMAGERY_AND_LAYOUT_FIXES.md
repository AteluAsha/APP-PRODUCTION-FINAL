...
**Cause:** The parallax header (hero background + chakra image) is in an `Animated.View` with **`zIndex: -1`**, so it renders *behind* the scroll content. The scroll content had **no top spacer**, so it started at y=0 and sat on top of the header. The header was fully covered and looked like a black bar.
