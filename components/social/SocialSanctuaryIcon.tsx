...
export const SocialSanctuaryIcon: React.FC<SocialSanctuaryIconProps> = ({ onPress }) => {
        ...
        <View className="absolute bottom-6 right-6 z-50">
            ...
            {showHint && !hasSeenHint && (
                <View className="absolute -top-20 right-0 bg-purple-900/90 px-4 py-2 rounded-lg border border-purple-700/50 max-w-[200px] mb-2">
                    <AppText font="instrument-regular" size="xs" className="text-white/90 text-center leading-4">
            ...
            <Animated.View style={animatedStyle}>
                ...
                >
                    <LinearGradient
                        colors={['#9D4EDD', '#7B2CBF', '#6A1B9A']} // Deep purple gradient matching Anua's energy
                        start={{ x: 0, y: 0 }}
                        end={{ x: 1, y: 1 }}
                        style={{
                            borderRadius: 32,
                            padding: 12,
                            borderWidth: 2,
                            borderColor: '#FFD700', // Gold border for sacred geometry
                            alignItems: 'center',
                    ...
                    >
                        <AnuaLogo size={40} animated={true} />
                    </LinearGradient>
                </Pressable>
            </Animated.View>
            
            {/* Subtle label */}
            <View className="absolute -top-8 right-0 bg-black/70 px-2 py-1 rounded">
                <AppText font="instrument-regular" size="xs" className="text-white">
                    Sanctuary
                </AppText>
            </View>
        </View>
