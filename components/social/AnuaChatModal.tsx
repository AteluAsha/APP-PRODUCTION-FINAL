...
As you prepare, I'd love to understand your starting point. How do you currently connect with your body's energy? Have you worked with meditation, breathwork, or somatic practices? What are you hoping to discover about yourself?`,
      ...
      // She speaks once automatically on opening (greeting only) - skip when starting from a note
      if (!initialMessage && isElevenLabsAvailable()) {
  ...
  }, [visible, useVoice, isWaitingRoom, initialMessage])
...
const AnuaChatContent: React.FC<{
  ...
  const inputRowH = 100
  const scrollMinHeight = Math.max(200, windowHeight - insets.top - insets.bottom - headerH - voiceToggleH - inputRowH)
      ...
      >
        ...
        >
          <View style={{ flex: 1, flexDirection: "row", alignItems: "center" }}>
            ...
            />
            <View style={{ flex: 1 }}>
              <AppText font="instrument-bold" size="xl" style={{ color: "#ffffff" }}>
                Talk to Anua
              </AppText>
              <AppText
        ...
        >
          <View style={{ flex: 1, minHeight: scrollMinHeight }}>
            ...
            {dailyTransmission && (
              <View style={[chatStyles.messageRow, chatStyles.messageRowLeft, { marginBottom: 24, paddingTop: 8 }]}>
                <View
                  style={[
                ...
                >
                  <LinearGradient
                    colors={["rgba(212, 165, 116, 0.95)", "rgba(180, 140, 100, 0.9)"]}
                    start={{ x: 0, y: 0 }}
                    end={{ x: 1, y: 0 }}
                    style={chatStyles.senderNameWrap}
                  >
                    <AppText font="instrument-semibold" size="xs" style={chatStyles.senderText}>
            ...
            <View
              ...
              style={[
                ...
                message.isUser ? chatStyles.messageRowRight : chatStyles.messageRowLeft,
              ]}
            >
              <View
                style={[
                  chatStyles.messageBubble,
                  message.isUser ? chatStyles.messageBubbleRight : chatStyles.messageBubbleLeft,
                ]}
              >
                <LinearGradient
                  colors={
                    message.isUser
                      ? ["rgba(135, 174, 115, 0.95)", "rgba(107, 142, 90, 0.9)"]
                      : ["rgba(212, 165, 116, 0.95)", "rgba(180, 140, 100, 0.9)"]
                  }
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 0 }}
                  style={chatStyles.senderNameWrap}
                ...
                <AppText font="instrument-regular" size="sm" style={chatStyles.messageText}>
                  {message.text}
                </AppText>
                <AppText
                  font="instrument-regular"
                  ...
                  style={{
                    marginTop: 6,
                    color: message.isUser ? "rgba(168, 201, 154, 0.9)" : "rgba(212, 197, 169, 0.8)",
                  }}
                >
          ...
          {isAnuaSpeaking && (
            <View style={[chatStyles.messageRow, chatStyles.messageRowLeft]}>
              <View style={[chatStyles.messageBubble, chatStyles.messageBubbleLeft]}>
                <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
                  <ActivityIndicator size="small" color="rgba(212, 165, 116, 0.9)" />
                  <AppText font="instrument-regular" size="sm" style={chatStyles.messageText}>
                    Anua is speaking...
                  </AppText>
                </View>
                <AppText font="instrument-regular" size="xs" style={{ marginTop: 6, color: "rgba(212, 197, 169, 0.8)" }}>
            ...
            <View style={[chatStyles.messageRow, chatStyles.messageRowLeft]}>
              <View
                style={[
                  chatStyles.messageBubble,
                  chatStyles.messageBubbleLeft,
                  { borderColor: "rgba(220, 38, 38, 0.4)", backgroundColor: "rgba(127, 29, 29, 0.25)" },
                ]}
              >
                <AppText font="instrument-regular" size="sm" style={{ color: "rgba(248, 113, 113, 0.98)" }}>
                  {error}
                </AppText>
              </View>
            </View>
        ...
        >
          <TextInput
            style={[chatStyles.input, { textAlignVertical: "top", minHeight: 52 }]}
            value={inputText}
            onChangeText={(text) => {
              const sanitized = text.replace(/[<>]/g, "")
              if (sanitized.length <= 500) setInputText(sanitized)
            }}
