                ...
                const chakrasPromises = Object.entries(DAY_TO_DOC_ID).map(
                    async ([dayStr, docId]) => {
                        ...
                        if (!chakraDocSnap.exists()) {
                            throw new Error(
                                `Chakra document ${docId} not found in Firestore`,
                            )
                        }
                        ...
                        const chakrasUsingEmbodimentAudio = ['solar_plexus', 'third_eye', 'crown']
                        if (data.audiopath && !chakrasUsingEmbodimentAudio.includes(docId)) {
                            if (!storage) {
